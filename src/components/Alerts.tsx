...
export function Alerts() {
  ...
  const [activeFilter, setActiveFilter] = useState<
    "all" | "expiration" | "receivable" | "scheduled_payment"
  >("all");
  const [alerts, setAlerts] = useState<RealAlertItem[]>([]);
  const [loading, setLoading] = useState(true);
  const [wallets, setWallets] = useState<any[]>([]);
  const [settleLedgerItem, setSettleLedgerItem] =
    useState<RealAlertItem | null>(null);
    ...
    unsubEntities = onSnapshot(qEnt, (snap) => {
      snap.forEach((doc) => {
        ...
        if (entData.name) {
          ...
          entitiesMap[entData.name.toLowerCase().trim()] =
            entData.contact || "";
        }
      });
    });
    ...
    const triggerSync = () => {
      ...
      unsubTxs = onSnapshot(qTxs, (txSnap) => {
        const txAlerts = txSnap.docs.map((d) => {
          ...
          const contactPhone =
            entitiesMap[intermediaryId] ||
            entitiesMap[intermediaryName.toLowerCase().trim()] ||
            "";
        ...
        unsubServices = onSnapshot(qSer, (serSnap) => {
          const rawServices = serSnap.docs.map(
            (doc) => ({ id: doc.id, ...doc.data() }) as any,
          );

          const now = new Date();
          const serAlerts = rawServices
            .filter((ser) => {
            ...
            .map((ser) => {
              ...
              const extraDetail =
                ser.email || ser.profileName || ser.category || "";
          ...
          unsubLedger = onSnapshot(qLedger, (ledgerSnap) => {
            ...
            const combined = [...txAlerts, ...serAlerts, ...ledgerAlerts].sort(
              (a, b) => {
  ...
  const handleSettlePayment = async () => {
    ...
    if (!settleSourceWalletId && !isWalletsDisabled) {
      alert(
        "Por favor seleccione una billetera de origen para realizar el pago.",
      );
      return;
    }
  ...
  return (
    <div className="space-y-6 lg:space-y-8 max-w-7xl mx-auto p-4 lg:p-8 text-left">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-end gap-6">
        ...
        <div
          className={cn(
            "flex flex-wrap gap-2 p-1 rounded-2xl w-full sm:w-auto shrink-0 border",
            isDark
        ...
        >
          ...
          <button
            onClick={() => setActiveFilter("scheduled_payment")}
            className={cn(
              "flex-1 sm:flex-none px-4 py-2 rounded-xl text-xs font-bold uppercase tracking-widest transition-all cursor-pointer",
          ...
          >
            Programados (
            {alerts.filter((a) => a.type === "scheduled_payment").length})
          </button>
        </div>
      </div>
              ...
              filteredAlerts.map((alert, idx) => (
                <motion.div
                  ...
                  className={cn(
                    ...
                    isDark
                      ? "bg-slate-900 border-slate-800 hover:border-indigo-900/50"
                    ...
                    <div
                      className={cn(
                        ...
                        alert.status === "expired"
                          ? isDark
                            ? "bg-rose-950/30 text-rose-500"
                            : "bg-rose-50 text-rose-500"
                          : alert.status === "past-due"
                            ? isDark
                              ? "bg-rose-950/30 text-rose-500"
                              : "bg-rose-50 text-rose-500"
                            : isDark
                      ...
                      <div className="flex flex-wrap items-center gap-2 mb-1">
                        <span className={cn("text-[10px] font-bold uppercase tracking-widest", isDark ? "text-white" : "text-black")}>
                          {alert.type === "expiration"
                            ? "Expiración Digital"
                            : alert.type === "receivable"
                              ? "Cobranza Trámite ANT"
                              : "Pago Programado Egreso"}
                        </span>
                        <span
                          className={cn(
                    ...
                    {alert.type === "scheduled_payment" ? (
                      <button
                        onClick={() => {
                          ...
                          const firstDebitWallet = wallets.find(
                            (w) => w.type !== "credit_card",
                          );
                          setSettleSourceWalletId(
                            firstDebitWallet?.id || wallets[0]?.id || "",
                          );
                        }}
                    ...
                    {alert.type !== "scheduled_payment" && (
                      <button
                        onClick={(e) => {
                          ...
                          if (!cleanPhone) {
                            window.alert(
                              "No hay teléfono de WhatsApp válido guardado para este cliente. Por favor regístrelo en CRM o en el Servicio Digital para automatizar.",
                            );
                            return;
                          }
                          ...
                          setNoticeShareData({
                            ...
                            title:
                              alert.type === "expiration"
                                ...
                                : "Notificación de Pago de Trámite",
                            subtitle:
                              alert.type === "expiration"
                                ? `Renovación de Servicio: ${alert.item}`
                                : `Regulación de Trámite ANT: ${alert.item}`,
                            items: [
                              {
                                concept: alert.item,
                                reference: alert.date
                                  ? `Vence: ${alert.date}`
                                  : undefined,
                                amount: amt,
                              },
                            ...
                            paymentInstructions:
                              alert.type === "expiration"
                                ? "Confírmanos si deseas conservar tu servicio de manera ininterrumpida."
                                : "Por favor ayúdanos con el depósito para regularizar tus trámites.",
                            type:
                              alert.type === "expiration"
                                ? "payable"
                                : "receivable",
      ...
      {settleLedgerItem && (
        <div className={cn("fixed inset-0 flex items-center justify-center p-4 z-50 animate-fade-in text-left backdrop-blur-sm", isDark ? "bg-slate-950/60" : "bg-slate-900/20")}>
          <motion.div
            ...
            >
              ...
              <div className="flex justify-between">
                ...
                <span className="text-xs font-mono font-black">
                  {settleLedgerItem.date || "Sin fecha"}
                </span>
              </div>
              ...
              <div className="pt-2 border-t border-slate-800/10 flex justify-between items-baseline">
                ...
                <span className="text-xl font-black text-rose-500">
                  {formatCurrency(settleLedgerItem.amount || 0)}
                </span>
              </div>
            </div>
