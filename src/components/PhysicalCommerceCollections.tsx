    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className={cn("text-2xl font-bold", isDark ? "text-white" : "text-black")}>{selectedClient.clientName}</h2>
            <p className={cn("text-sm font-semibold", isDark ? "text-white" : "text-black")}>CI: {selectedClient.cedula}</p>
          </div>
        </div>
        ...
        <div className="flex flex-wrap gap-4 items-center justify-between p-4 bg-indigo-50 dark:bg-indigo-900/20 rounded-2xl border border-indigo-100 dark:border-indigo-800/50">
          <div>
            ...
            <p className={cn("text-3xl font-black", isDark ? "text-white" : "text-black")}>{formatCurrency(selectedClient.totalPending)}</p>
          </div>
          <div className="flex gap-2">
            <button 
              onClick={handleExportPDF}
              className={cn("flex items-center gap-2 px-4 py-2 border rounded-xl font-bold shadow-sm transition-colors", isDark ? "bg-slate-800 border-slate-700 text-white hover:bg-slate-700" : "bg-white border-slate-200 text-black hover:bg-slate-50")}
            >
        ...
        <div className={cn("flex p-1 rounded-xl", isDark ? "bg-slate-800/50" : "bg-slate-100")}>
          <button
            onClick={() => setClientTab("pending")}
            className={cn(
              "flex-1 py-2 text-sm font-bold rounded-lg transition-all",
              clientTab === "pending" ? (isDark ? "bg-slate-700 text-white shadow" : "bg-white text-black shadow") : (isDark ? "text-slate-300 hover:text-white" : "text-slate-700 hover:text-black")
            )}
          >
            Cuotas Pendientes
          </button>
          <button
            ...
            className={cn(
              "flex-1 py-2 text-sm font-bold rounded-lg transition-all",
              clientTab === "history" ? (isDark ? "bg-slate-700 text-white shadow" : "bg-white text-black shadow") : (isDark ? "text-slate-300 hover:text-white" : "text-slate-700 hover:text-black")
            )}
          >
            Historial de Pagos
          </button>
              ...
              {pendingInstallments.length > 0 ? pendingInstallments.map((inst, idx) => {
                ...
                return (
                  <div key={`${inst.saleId}-${idx}`} className={cn(
                    ...
                    isDark ? "bg-slate-800/50 border-slate-700 text-white" : "bg-white border-slate-200 text-black",
                    isOverdue && (isDark ? "border-red-900/50 bg-red-900/10" : "border-red-200 bg-red-50")
                  )}>
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        <span className={cn("font-black text-lg", isDark ? "text-white" : "text-black")}>Cuota #{inst.installmentNumber}</span>
                      ...
                      <p className={cn("text-sm font-semibold flex items-center gap-2", isDark ? "text-white" : "text-black")}>
                        <Calendar className="w-3.5 h-3.5" />
                        Vence: {dueDate.toLocaleDateString()}
                      </p>
                      <p className={cn("text-xs font-medium mt-1 line-clamp-1", isDark ? "text-white" : "text-black")}>
                ...
                <div className={cn("text-center py-8", isDark ? "text-white" : "text-black")}>
                  <CheckCircle className="w-12 h-12 mx-auto mb-2 text-emerald-500" />
                  <p className="font-bold">Este cliente no tiene cuotas pendientes.</p>
                </div>
              )}
            </div>
          </div>
          ...
          <div className="space-y-4">
            <div className="grid gap-3">
              {paymentHistory.length > 0 ? paymentHistory.map((payment, idx) => (
                <div key={idx} className={cn(
                  ...
                  <div>
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className={cn("text-xs font-bold", isDark ? "text-white" : "text-black")}>• Cuota #{payment.installmentNumber}</span>
                    </div>
                    <p className={cn("text-sm font-semibold", isDark ? "text-white" : "text-black")}>
                      {new Date(payment.date).toLocaleString()}
                    </p>
                    {payment.comment && (
                      <p className={cn("text-xs italic font-medium mt-1", isDark ? "text-white" : "text-black")}>"{payment.comment}"</p>
                    )}
                  </div>
                  <div className="flex items-center gap-4">
                    <p className={cn("text-xl font-black", isDark ? "text-white" : "text-black")}>
                ...
                <div className={cn("text-center py-8", isDark ? "text-white" : "text-black")}>
                  <History className="w-12 h-12 mx-auto mb-2 text-indigo-500" />
                  <p className="font-bold">No hay historial de pagos registrado.</p>
                </div>
              )}
            </div>
          </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
        <div>
          <h2 className={cn("text-xl font-black", isDark ? "text-white" : "text-black")}>Cobranza Inteligente</h2>
          <p className={cn("text-sm font-semibold", isDark ? "text-white" : "text-black")}>Gestiona las cuentas por cobrar y el historial de pagos de tus clientes.</p>
        </div>
        <div className="w-full sm:w-auto flex-1 max-w-sm relative">
          <Search className={cn("w-5 h-5 absolute left-3 top-1/2 -translate-y-1/2", isDark ? "text-white" : "text-black")} />
          <input 
            type="text"
            ...
            className={cn(
              "w-full pl-10 pr-4 py-2.5 rounded-xl border text-sm font-bold transition-all focus:ring-2",
              isDark ? "bg-slate-800 border-slate-700 text-white focus:ring-indigo-500/50" : "bg-white border-slate-200 text-black focus:ring-indigo-500/30"
            )}
          />
        </div>
      </div>
      ...
      <div className="grid gap-3">
        {filteredClients.map(client => (
          <div 
            ...
            className={cn(
              ...
              isDark ? "bg-slate-800/50 border-slate-700 hover:bg-slate-800 text-white" : "bg-white border-slate-200 hover:bg-slate-50 text-black"
            )}
          >
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className={cn("font-bold text-base", isDark ? "text-white" : "text-black")}>{client.clientName}</h3>
                <p className={cn("text-xs font-semibold", isDark ? "text-white" : "text-black")}>CI: {client.cedula} • {client.installments.filter(i => i.status !== 'paid').length} cuotas pendientes</p>
              </div>
            </div>
            <div className="flex items-center gap-4">
              <div className="text-right">
                <p className={cn("text-xs font-black uppercase tracking-wider mb-0.5", isDark ? "text-white" : "text-black")}>Deuda Total</p>
        ...
        {filteredClients.length === 0 && (
          <div className={cn("text-center py-12", isDark ? "text-white" : "text-black")}>
